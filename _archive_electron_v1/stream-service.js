...
async function applyStreamConfigToObs(wsConfig = {}) {
  try {
    ...
    const expectedServer = buildRtmpServer(currentConfig);
    const expectedKey = currentConfig.streamKey;
    ...
    const applied = await obs.call('GetStreamServiceSettings');
    const settings = applied.streamServiceSettings || {};
    const actualServer = String(settings.server || '');
    const actualKey = String(settings.key || '');

    if (actualServer !== expectedServer || actualKey !== expectedKey) {
      return {
        ok: false,
        error: 'OBS 推流配置写入后读回不一致，请检查 OBS WebSocket 权限或 OBS 当前配置状态',
        expected: { server: expectedServer, key: expectedKey },
        actual: { server: actualServer, key: actualKey },
      };
    }
