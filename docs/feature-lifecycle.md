...
新功能进入实现前，先按 `docs/feature-evaluation-guide.md` 完成立项评估、范围定义、任务拆解、验收标准和回滚方案。
...
测试只能跟随明确的产品行为或接口契约变化而调整。不得为了让当前实现通过而擅自删除断言、跳过用例、扩大 mock、降低 schema 校验或放宽安全边界。特殊情况必须在变更说明中写清原因、风险和替代验证方式。
