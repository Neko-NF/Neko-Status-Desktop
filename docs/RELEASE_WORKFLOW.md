...
发布说明默认面向普通用户，而不是开发者或维护者。除非本次发布是专门给开发者使用的调试版本，否则只写用户能感知到的新增、优化、修复和必要注意事项。
...
- 不写 SHA256 校验命令
- 不写和用户无关的内部噪音
- 不展开内部技术细节、测试文件、IPC 名称、重构路径或实现过程
- 文案保持简短，优先写“用户会看到什么变化”或“问题是否解决”
- 需要说明限制时用普通用户能理解的话，例如“部分设置需要重启后生效”
