...
- 正式版和开发版使用不同的命名管道、互斥锁、配置目录和自启动项；单个版本通道内仍保持单实例。
...
Activity 健康状态使用完整快照而不是一个可变字符串：生命周期、本地 IPC、凭据、事件接收和 Presence 发布分别建模，`overall` 只做派生。IPC v1 保持严格请求—响应；Renderer 通过 revision 拒绝过期快照。详细契约和线上恢复边界见 [连接健康状态与服务端恢复清单](./用户关注与应用在线提醒/10_连接健康状态与服务端恢复清单.md)。
