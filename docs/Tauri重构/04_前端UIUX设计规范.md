...
加载反馈按任务语义选择，不按页面偏好选择：显眼的非确定等待使用数学曲线，按钮使用紧凑 busy，结构化首屏使用骨架屏，可计算任务使用进度条、百分比与速度，静默后台任务只使用状态徽章或文字。

- 曲线动画只能服务加载反馈，不得成为页面背景、空态插画或常驻装饰。
- 曲线运动必须有明确加载语义；新增预设不得默认匀速绕圈或整体旋转，应声明非线性速度、往返/开放/循环等运动性格。
- 动画旁必须有明确状态文字，不能只依赖颜色或运动表达状态。
- 未来新增曲线必须通过统一注册表，使用稳定 ID，并通过有限采样、归一化、闭合/淡出测试。
- `prefers-reduced-motion: reduce` 下不启动帧循环，仅显示静态曲线。
- forced colors 下使用 `currentColor`，确保高对比模式可识别。
- 同一正式界面最多动画化 4 个实例；画廊缩略图必须静态。
- 确定进度始终优先于曲线动画，曲线不得遮盖百分比、速度或剩余大小。

详细语义矩阵、发布与回滚门禁见 [数学曲线加载系统与 UI 实验室](../math-curve-loading-system.md)。
